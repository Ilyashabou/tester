    ...
    def extract_elements_by_role(self, soup):
        ...
        try:
            ...
            for element in all_inputs:
                ...
                # Empty input_type means it's a text input by default
                if input_type == 'checkbox':
                    ...
                    role = 'button'
                    action = 'click'
                else:  # text, email, password, search, tel, url, etc. or empty (default to text)
                    role = 'input'
                    action = 'fill'
            ...
            if role in elements_by_role:
                ...
                for element in elements_by_role[role]:
                    ...
                    if role in ['button', 'link', 'form'] or action == 'click':
                        ...
                        test_steps.append(f"    # Wait a moment for any visual changes to complete")
                        test_steps.append(f"    page.wait_for_timeout(500)  # 500ms wait")
                        ...
                        test_steps.append(f"    # Detect visual changes")
                        test_steps.append(f"    visual_changed = detect_visual_change(before_screenshot, after_screenshot)")
                        test_steps.append(f"    if visual_changed:")
                        test_steps.append(f"        print(f'Visual changes detected after interacting with {role}: {display_text}')")
                        ...
                        test_steps.append(f"        visual_change_detected=visual_changed")
                        test_steps.append(f"    )")
...
def test_page():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            session_loaded = session_manager.load_session(context, "{url}")
            if session_loaded:
