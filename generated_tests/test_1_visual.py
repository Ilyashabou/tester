...
# Playwright test for https://thinking-tester-contact-list.herokuapp.com/contactList
# Generated using role-based template approach
# HTML file: html_files\https___thinking_tester_contact_list_herokuapp_com_contactList_after_auth.html
...
def test_page():
    """
    Role-based test for https://thinking-tester-contact-list.herokuapp.com/contactList
    ...
    with sync_playwright() as p:
        browser = None
        try:
            print(f"Starting test for https://thinking-tester-contact-list.herokuapp.com/contactList")
            ...
            session_loaded = session_manager.load_session(context, "https://thinking-tester-contact-list.herokuapp.com/contactList")
            if session_loaded:
            ...
            original_url = "https://thinking-tester-contact-list.herokuapp.com/contactList"

            # Navigate to the URL with improved error handling
            print(f"Navigating to https://thinking-tester-contact-list.herokuapp.com/contactList")
            try:
                page.goto("https://thinking-tester-contact-list.herokuapp.com/contactList", timeout=30000, wait_until="networkidle")
                ...
                element_tracker.record_element_test(
                    page_url="https://thinking-tester-contact-list.herokuapp.com/contactList",
                    element_type="page",
                    selector="N/A",
...
# Testing button elements
            try:
                ...
                before_screenshot = take_screenshot(page, 'button_5_before')
                ...
                after_screenshot = take_screenshot(page, 'button_18_after')
            ...
            try:
                print(f'Testing button: Add a New Contact')
                # Take screenshot before interaction
                before_screenshot = take_screenshot(page, 'button_58_before')
                # Record URL and title before interaction
                before_url = page.url
                before_title = page.title()
                # Perform the action
                page.locator('#add-contact').click(timeout=3000)
                try:
                    page.wait_for_load_state('networkidle', timeout=5000)
                except Exception as e:
                    print(f'Navigation wait error: {{e}}')
                ...
                page_changed = after_url != before_url or after_title != before_title
                # Take screenshot after interaction
                after_screenshot = take_screenshot(page, 'button_71_after')
                # Detect visual changes
                visual_changed = detect_visual_change(before_screenshot, after_screenshot)
                if visual_changed:
                    print(f'Visual changes detected after interacting with button: Add a New Contact')
                # Record element effectiveness
                element_tracker.record_element_test(
                    page_url=original_url,
                    element_type='button',
