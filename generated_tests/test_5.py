...
# Playwright test for https://thinking-tester-contact-list.herokuapp.com/login
# Generated using role-based template approach
# HTML file: html_files\https___thinking_tester_contact_list_herokuapp_com_login.html
...
def test_page():
    """
    Role-based test for https://thinking-tester-contact-list.herokuapp.com/login
    ...
    with sync_playwright() as p:
        browser = None
        try:
            print(f"Starting test for https://thinking-tester-contact-list.herokuapp.com/login")
            ...
            session_loaded = session_manager.load_session(context, "https://thinking-tester-contact-list.herokuapp.com/login")
            if session_loaded:
            ...
            original_url = "https://thinking-tester-contact-list.herokuapp.com/login"

            # Navigate to the URL with improved error handling
            print(f"Navigating to https://thinking-tester-contact-list.herokuapp.com/login")
            try:
                page.goto("https://thinking-tester-contact-list.herokuapp.com/login", timeout=30000, wait_until="networkidle")
                ...
                element_tracker.record_element_test(
                    page_url="https://thinking-tester-contact-list.herokuapp.com/login",
                    element_type="page",
                    selector="N/A",
            ...
            # Execute test steps for each element type
                        ...
                        # Testing form with selector form
...
# Testing button elements
            try:
                ...
                # Take screenshot before interaction
                before_screenshot = take_screenshot(page, 'button_80_before')
                ...
                # Take screenshot after interaction
                after_screenshot = take_screenshot(page, 'button_93_after')
                # Detect visual changes
                visual_changed = detect_visual_change(before_screenshot, after_screenshot)
                if visual_changed:
                ...
                # Continue with other elements
            try:
                print(f'Testing button: Sign up')
                # Take screenshot before interaction
                before_screenshot = take_screenshot(page, 'button_133_before')
                # Record URL and title before interaction
                before_url = page.url
                before_title = page.title()
                # Perform the action
                page.locator('#signup').click(timeout=3000)
                try:
                    page.wait_for_load_state('networkidle', timeout=5000)
                except Exception as e:
                    print(f'Navigation wait error: {{e}}')
                ...
                page_changed = after_url != before_url or after_title != before_title
                # Take screenshot after interaction
                after_screenshot = take_screenshot(page, 'button_146_after')
                # Detect visual changes
                visual_changed = detect_visual_change(before_screenshot, after_screenshot)
                if visual_changed:
                    print(f'Visual changes detected after interacting with button: Sign up')
                # Record element effectiveness
                element_tracker.record_element_test(
                    page_url=original_url,
                    element_type='button',
...
# Testing link elements
            try:
                print(f'Testing link: here')
                # Take screenshot before interaction
                before_screenshot = take_screenshot(page, 'link_187_before')
                # Record URL and title before interaction
                before_url = page.url
                before_title = page.title()
                # Perform the action
                page.locator("a[href='https://documenter.getpostman.com/view/4012288/TzK2bEa8']:has-text('here')").click(timeout=3000)
                try:
                    page.wait_for_load_state('networkidle', timeout=5000)
                except Exception as e:
                    print(f'Navigation wait error: {{e}}')
                ...
                page_changed = after_url != before_url or after_title != before_title
                # Take screenshot after interaction
                after_screenshot = take_screenshot(page, 'link_200_after')
                # Detect visual changes
                visual_changed = detect_visual_change(before_screenshot, after_screenshot)
                if visual_changed:
                    print(f'Visual changes detected after interacting with link: here')
                # Record element effectiveness
                element_tracker.record_element_test(
                    page_url=original_url,
                    element_type='link',
                    selector="a[href='https://documenter.getpostman.com/view/4012288/TzK2bEa8']:has-text('here')",
                    description='here',
                    success=True,  # We got this far without exception
                    screenshot_before=before_screenshot,
                    screenshot_after=after_screenshot,
