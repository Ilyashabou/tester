...
# Playwright test for https://example.com
# Generated using role-based template approach
# HTML file: html_files\https___example_com.html
...
def test_page():
    """
    Role-based test for https://example.com
    ...
    with sync_playwright() as p:
        browser = None
        try:
            print(f"Starting test for https://example.com")
            ...
            session_loaded = session_manager.load_session(context, "https://example.com")
            if session_loaded:
            ...
            print(f"Navigating to https://example.com")
            try:
...
# Testing link elements
            try:
                print(f'Testing link: More information...')
                # Take screenshot before interaction
                before_screenshot = take_screenshot(page, 'link_6_before')
                # Record URL and title before interaction
                before_url = page.url
                before_title = page.title()
                # Perform the action
                page.locator("a[href='https://www.iana.org/domains/example']:has-text('More information...')").click(timeout=3000)
                try:
                ...
                after_screenshot = take_screenshot(page, 'link_19_after')
                # Detect visual changes
                visual_changed = detect_visual_change(before_screenshot, after_screenshot)
                if visual_changed:
                    print(f'Visual changes detected after interacting with link: More information...')
                # Record element effectiveness
                element_tracker.record_element_test(
                    page_url=original_url,
                    element_type='link',
                    selector="a[href='https://www.iana.org/domains/example']:has-text('More information...')",
